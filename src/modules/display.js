...
function createTableHeaders() {
  ...
  tableHeaderMonth.innerText =
    settings.displayTable === 'month' ? 'Month' : 'Year'
...
function populateTable(theCalculationsResults) {
  ...
  theCalculationsResults.forEach(result => {
    ...
    tableDataMonthOrYear.innerText =
      settings.displayTable === 'month' ? result.month : result.year
    tableRow.append(tableDataMonthOrYear)

    const tableDataInterestPaid = document.createElement('td')
    tableDataInterestPaid.innerText =
      settings.displayTable === 'month'
        ? formatNumbersAsCurrency(result.interestPaid)
        : formatNumbersAsCurrency(result.interestPaidThisYear)

    tableRow.append(tableDataInterestPaid)

    const tableDataTotalInterestPaid = document.createElement('td')
    tableDataTotalInterestPaid.innerText =
      settings.displayTable === 'month'
        ? formatNumbersAsCurrency(result.totalInterestPaidToDate)
        : formatNumbersAsCurrency(result.totalInterestPaid)
    tableRow.append(tableDataTotalInterestPaid)

    const tableDataOutstandingMortgageAmount = document.createElement('td')
    tableDataOutstandingMortgageAmount.innerText = formatNumbersAsCurrency(
...
function displayTextResults(
  ...
  const timeToBeDebtFree =
    savings.monthsSaved > 12
      ? `${Math.floor(savings.monthsSaved / 12)} ${yearsText} and ${
          savings.monthsSaved % 12
        } ${monthsText}`
      : `${savings.monthsSaved} ${monthsText}`

  const resultsToDisplay = `Your usual monthly payment is <span>${formatNumbersAsCurrency(
    normalMonthlyPayment
  )}</span>. By overpaying <span>${formatNumbersAsCurrency(
    monthlyOverpayments
  )}</span> a month on your mortgage with a debt of <span>${formatNumbersAsCurrency(
    mortgageAmount
  )}</span> and an interest rate of <span>${interestRate}%</span>, you could save <span>${formatNumbersAsCurrency(
...
function switchDisplay() {
  ...
  function recalculateResulsts() {
    if (
      document.getElementById('calculate-button').className === 'resulsts-exist'
    ) {
