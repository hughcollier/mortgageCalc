...
function getUserInput() {
  ...
  submitBtn.addEventListener('click', event => {
    ...
    monthlyOverpayments = document.getElementById(
    ...
    const monthlyOverpaymentsError = document.getElementById(
      'monthly-overpayments'
    )
    ...
    if (monthlyOverpayments > (mortgageAmount / 100) * 10) {
      errorMessage.push(
        'overpayments cannot be more than 10% of mortage amount'
      )
      monthlyOverpaymentsError.classList.add('form-error', 'animate__headShake')
    }
    ...
    if (errorMessage.length > 0) {
      errorMessage[0] =
        errorMessage[0].charAt(0).toUpperCase() + errorMessage[0].slice(1)

      errorMessage = errorArrayFormatting(errorMessage)
      formValidatoin.append(errorParagraph)
      errorParagraph.innerText = errorMessage
    } else {
      let theCalculationsResults = calculateMonthlyRecords(
        years,
        interestRate,
        mortgageAmount,
        monthlyOverpayments
      ...
      const theCalculationsResultsForComparison = calculateMonthlyRecords(
        years,
        interestRate,
        mortgageAmount,
        0
      )
      const savings = calculateMoneyAndTimeSaved(
        theCalculationsResults,
        theCalculationsResultsForComparison
      )
      ...
      if (settings.displayTable === 'year') {
        theCalculationsResults = createMortgageObjectForYearDisplay(
          theCalculationsResults
        )
      }
